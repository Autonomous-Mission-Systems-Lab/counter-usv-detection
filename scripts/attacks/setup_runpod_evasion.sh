...
echo "[setup-evasion] repo=${REPO_ROOT}"

if [[ ! -d "${VENV}" ]]; then
  echo "[setup-evasion] ERROR: missing ${VENV}" >&2
...
python - <<'PY'
import torch
print(f"[setup-evasion] torch={torch.__version__} cuda={torch.cuda.is_available()}"
      f" device={torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'cpu'}")
if not torch.cuda.is_available():
    raise SystemExit(
        "[setup-evasion] ERROR: CUDA not available — provision a CUDA RunPod "
...
for f in "${need[@]}"; do
  if [[ ! -e "${f}" ]]; then
    echo "[setup-evasion] ERROR: missing ${f}" >&2
    echo "  Sync EO bundle + evasion overlay (see docs/RUNPOD.md § Evasion)." >&2
    exit 1
  fi
done
...
from counterusv.attacks.evasion import DifferentiableSurrogate, to_torch_device
fam = "${FAMILY}"
b = DetectorBaseline.from_freeze(fam, device="0")
print(f"[setup-evasion] freeze OK: {fam} weights={b.weights}")
dev = to_torch_device("auto")
print(f"[setup-evasion] torch device from auto: {dev}")
...
print(f"[setup-evasion] raw head OK: boxes={tuple(boxes.shape)} scores={tuple(scores.shape)}")
...
print("[setup-evasion] CUDA autograd OK")
PY

echo "[setup-evasion] evasion dry-run (slice wiring, no optimize)"
python scripts/attacks/run_evasion.py --family "${FAMILY}" --device 0 --dry-run

echo
echo "[setup-evasion] ready. Full ESR run (survive SSH disconnect):"
echo "  apt-get update -qq && apt-get install -y -qq tmux   # once if missing"
echo "  source .venv/bin/activate"
echo "  tmux new -s evasion"
echo "  # inside tmux:"
echo "  python scripts/attacks/run_evasion.py --family ${FAMILY} --device 0"
echo "  # detach: Ctrl-b then d"
echo "  # optional smoke first:"
echo "  python scripts/attacks/run_evasion.py --family ${FAMILY} --device 0 --max-images 2 --steps 30"
echo
echo "Outputs → results/attacks/evasion/${FAMILY}/"
