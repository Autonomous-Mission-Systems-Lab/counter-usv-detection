#!/usr/bin/env bash
# Sync the evasion-attack overlay to a RunPod workspace.
#
...
LIST="${REPO_ROOT}/configs/attacks/runpod_evasion_paths.txt"
...
echo "[sync-evasion] repo=${REPO_ROOT}"
echo "[sync-evasion] host=${REMOTE_HOST}"
echo "[sync-evasion] remote_root=${REMOTE_ROOT}"
echo "[sync-evasion] rsh=${RSYNC_RSH}"

if ! remote_sh "command -v rsync >/dev/null 2>&1"; then
  echo "[sync-evasion] remote has no rsync — installing…"
  if ! remote_sh "if command -v apt-get >/dev/null; then apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq rsync; elif command -v yum >/dev/null; then yum install -y -q rsync; else exit 1; fi"; then
    echo "[sync-evasion] ERROR: could not install rsync on the pod." >&2
    exit 1
  fi
fi
...
echo "[sync-evasion] ${#PATHS[@]} paths to sync"

sync_one() {
  local rel="$1" src="${REPO_ROOT}/$1"
  if [[ ! -e "${src}" ]]; then
    echo "[sync-evasion] SKIP missing: ${rel}" >&2
    return 0
  fi
  echo "[sync-evasion] ${rel}"
  if [[ -d "${src}" ]]; then
...
echo
echo "[sync-evasion] done. On the pod:"
echo "  cd ${REMOTE_ROOT}"
echo "  bash scripts/attacks/setup_runpod_evasion.sh"
echo "  # then (inside tmux):"
echo "  python scripts/attacks/run_evasion.py --family yolo11s --device 0"
