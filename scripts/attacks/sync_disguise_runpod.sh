#!/usr/bin/env bash
# Sync the disguise-attack overlay to a RunPod workspace.
#
# Run on your LAPTOP. The EO imagery + annotations must already be on the pod
# (from scripts/detector/sync_eo_train_bundle.sh). This only pushes attack code
# + frozen surrogate weights (~70 MB).
#
# Preferred (RunPod Connect → Direct TCP IP + port):
...
#
# Fresh pod (no EO data yet)? Sync the EO bundle FIRST, then this overlay:
...
LIST="${REPO_ROOT}/configs/attacks/runpod_disguise_paths.txt"
...
echo "[sync-disguise] repo=${REPO_ROOT}"
echo "[sync-disguise] host=${REMOTE_HOST}"
echo "[sync-disguise] remote_root=${REMOTE_ROOT}"
echo "[sync-disguise] rsh=${RSYNC_RSH}"

if ! remote_sh "command -v rsync >/dev/null 2>&1"; then
  echo "[sync-disguise] remote has no rsync — installing…"
  if ! remote_sh "if command -v apt-get >/dev/null; then apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq rsync; elif command -v yum >/dev/null; then yum install -y -q rsync; else exit 1; fi"; then
    echo "[sync-disguise] ERROR: could not install rsync on the pod." >&2
    exit 1
  fi
fi
...
echo "[sync-disguise] ${#PATHS[@]} paths to sync"

sync_one() {
  local rel="$1" src="${REPO_ROOT}/$1"
  if [[ ! -e "${src}" ]]; then
    echo "[sync-disguise] SKIP missing: ${rel}" >&2
    return 0
  fi
  echo "[sync-disguise] ${rel}"
  if [[ -d "${src}" ]]; then
...
echo
echo "[sync-disguise] done. On the pod:"
echo "  cd ${REMOTE_ROOT}"
echo "  bash scripts/attacks/setup_runpod_disguise.sh"
echo "  # then (inside tmux):"
echo "  python scripts/attacks/run_disguise.py --family yolo11s --device 0"
