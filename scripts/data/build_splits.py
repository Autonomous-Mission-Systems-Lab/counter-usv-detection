...
images/tracks. Two independent partitions, each with its own leakage rule:
...
# --------------------------------------------------------------------------- checks
def leakage_checks(eo, ais, train_only_sources) -> dict:
    problems = []
...
def summarize(eo, ais, train_only_sources, eval_sources, ratios, checks, versions):
    def dist(df, by):
...
def main(argv=None) -> int:
    ...
    checks = leakage_checks(eo, ais, set(train_only))
    ...
    summary = summarize(eo, ais, train_only, eval_sources, ratios, checks, versions)
    ...
    stale_video = out_dir / "video_eval_pool.csv"
